# 전사 자산관리 시스템 — 아키텍처 설계 문서

> 작성 2026-05-14. 현재 시스템 구조·DB 설계 의도·ERP 연동 마이그레이션 전략을 기록한다.
...
현재 단계(1단계)는 **월별 집계 데이터를 CSV로 업로드해서 시각화**하는 구조다.
향후 단계(2단계)에서 ERP 시스템과 직접 연동해 실시간 데이터를 받도록 전환한다.

```
현재(1단계):  담당자가 CSV 업로드 → DB 집계 갱신 → 대시보드 표시
향후(2단계):  ERP 트랜잭션 발생 → 자동 수집 → 대시보드 실시간 반영
```
...
현장 변화를 그때그때 알리는 알림 기능. 두 단계로 구현한다.

**인앱 알림 (1단계, 즉시 구현 가능)**

```
DB: notifications 테이블
  (id, userId, title, body, link, readAt, createdAt)

API: 백그라운드 cron 잡 (node-cron, 매일 새벽)
  - 임대계약 만료 D-60 / D-30 / D-7
  - 임대율 85% 이하 건물 발생
  - 비품 감가상각 완료 예정 목록
  → 조건 충족 시 notifications INSERT

프론트: 헤더 알림 벨 → 읽지 않은 건 카운트 뱃지 → 목록 드롭다운
```

**브라우저/모바일 Push (2단계, 현장 검증 후)**
- Web Push API + 서비스 워커
- 앱을 열지 않아도 알림 수신
- 인앱 알림에서 어떤 알림이 실제로 유용한지 확인한 뒤 선별 적용
...
```sql
-- audit_logs: UPDATE/DELETE를 DB 정책으로 차단
-- 어떤 사용자도(admin 포함) 로그를 삭제·수정 불가
...
```
매월 말 / 데이터 업로드 시 → 전체 자산 상태를 타임스탬프와 함께 저장
용도: "2026년 1월 시점의 건물 장부가액은?" 즉시 조회
비교 UI: 두 시점 선택 → 자산 변동 diff 표시
```
...
현재: 화면 인쇄만 지원
추가:
  - 기간 선택 → audit_logs 필터 → PDF 다운로드
  - 파일 SHA-256 해시 첨부 → 위변조 여부 외부 검증 가능
