...
export function AssetDrilldownSubtab({ m0 }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 items-start md:grid-cols-[280px_minmax(0,1fr)]">
      {/* ── 좌: 3단 네비 ── */}
...
function LocationTable({ item }: { item: GroupItem }) {
  ...
  return (
    <table className="w-full border-collapse text-caption">
      ...
      <tbody>
        {locs.map(([loc, val, toneClass]) => (
          <tr key={loc} className="border-t border-border hover:bg-muted/40">
            <td className={cn('px-3 py-2 font-medium', toneClass)}>{loc}</td>
